...
轻体记 — 身材管理 App，面向不同体重人群，集成饮食指导与运动方案功能。

## 技术栈

- 前端：Expo + React Native + Uniwind (Tailwind v4)
- 后端：Express.js + TypeScript
- 路由：Expo Router (Tabs 导航，5 个 Tab)
- 图标：FontAwesome6
...
├── home/          # 首页（仪表盘、今日摘要）
├── diet/          # 饮食管理（三餐方案、食物列表）
├── exercise/      # 运动指导（分类筛选、运动列表）
├── record/        # 记录（饮食记录、运动统计）
├── profile/       # 我的（人群选择、个人信息）
├── food-detail/   # 食物详情
├── exercise-detail/ # 运动详情
├── meal-detail/   # 餐食详情（含制作方法）
└── heat-query/    # 热量查询

server/src/
├── data.ts        # 静态数据（食物、运动、人群、餐食方案）
└── routes/
    ├── foods.ts   # 食物 + 餐食 API
    ├── exercises.ts # 运动 API
    └── records.ts # 记录 + 用户 + 人群 API
```

## 路由模式

Tabs 导航：首页 / 饮食 / 运动 / 记录 / 我的
子页面：food-detail、exercise-detail、meal-detail、heat-query

## API 端点

- GET /api/v1/foods - 食物列表（支持 search/category 筛选）
- GET /api/v1/foods/categories - 食物分类列表
- GET /api/v1/foods/:id - 食物详情
- GET /api/v1/meals - 餐食方案列表
- GET /api/v1/meals/:id - 餐食详情（含制作方法）
- GET /api/v1/exercises - 运动列表（支持 category/bodyPart 筛选）
- GET /api/v1/exercises/categories - 运动分类
- GET /api/v1/exercises/body-parts - 身体部位
- GET /api/v1/exercises/:id - 运动详情
- GET /api/v1/populations - 人群列表
- GET /api/v1/populations/:id - 人群详情
- GET /api/v1/records/diet - 饮食记录
- POST /api/v1/records/diet - 创建饮食记录
- GET /api/v1/records/exercise - 运动记录
- POST /api/v1/records/exercise - 创建运动记录
- GET /api/v1/records/exercise/stats - 运动统计
- GET /api/v1/user/profile - 用户信息
- PUT /api/v1/user/profile - 更新用户信息
- GET /api/v1/user/dashboard - 首页仪表盘数据
